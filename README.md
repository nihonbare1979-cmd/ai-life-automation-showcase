...
プログラミングのできない会社員「葉月」が、約4ヶ月（2026年5月末〜）で16の自動化システムを作り、家族の毎日の暮らしの中で運用しているものの紹介ページです。
リベシティでは「葉月」として活動しています。作り方の手順は [note](https://note.com/hazuki_ai_home) で公開中です。

> 私はプログラマーではありません。コードはすべて AI（[Claude Code](https://claude.com/claude-code)）に生成させ、私は **課題の整理・要件定義・設計・動作検証・運用と改善** を担当しています。このリポジトリは「何を解決し、どう動いているか」を、同じように家の事務を自動化したい方に見ていただくためのもので、個人情報を含む実データや設定値は一切含めていません。

| | |
|---|---|
| 稼働中の自動化システム | **16** |
| 自動で動く定期タスク・常駐監視 | **38本**（毎朝・毎晩・毎週・毎月） |
| 繰り返し使える手順書（スキル） | **22** |
| AIを触り始めてから | **約4ヶ月**（2026-05-27 〜） |
...
| 4 | 不用品販売を「撮る・選ぶ」だけで下書きまで自動化 | 撮影・説明文・相場調査・複数サイトへの入力 | [systems/04-flea-market-autolist.md](systems/04-flea-market-autolist.md) | [▶ 試す](https://nihonbare1979-cmd.github.io/ai-life-automation-showcase/demos/04-flea-market-autolist/) |
...
| [不用品の自動出品](https://nihonbare1979-cmd.github.io/ai-life-automation-showcase/demos/04-flea-market-autolist/) | 元写真 → 状態を6択で選ぶ → AI分析 → サムネイル → 2サイト入力・検収 → 下書き保存 | 画像・商品情報はすべて本物の生成物、フォームは再現 |
...
- 子供のテスト写真から弱点を分析し、週次ドリルPDFを自動生成・印刷（第15回まで継続）
...
*すべて家族のための個人活動として、Claude Code を使って作り、運用しているものです。ご連絡はリベシティ（葉月）または [note](https://note.com/hazuki_ai_home) からどうぞ。*
