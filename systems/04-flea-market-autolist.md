...
> 実際にこの仕組みで出品した私物（USBワイヤレスアダプタ）を手に持って引きで撮った元写真から、AI生成の商品情報・構図調整後の写真（ビフォー／アフター比較つき）・サムネイルを処理の順に表示し、3サイトのフォーム自動入力を再現します。
> （GitHub Pages で公開。リポジトリ内のファイルは [`demos/04-flea-market-autolist/`](../demos/04-flea-market-autolist/index.html)）

## 実際の画面

![デモ画面：元写真 → AI分析 → 構図調整（前後比較・検出枠つき）→ サムネイル → 3サイトのフォーム入力（すべて本物の生成物）](../assets/04_demo_screen.png)

*デモ画面：元写真 → AI分析 → 構図調整（前後比較・検出枠つき）→ サムネイル → 3サイトのフォーム入力（すべて本物の生成物）*
